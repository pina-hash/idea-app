...
**BUILD 214 MOVED THE BUILD NUMBER, AND THIS FILE IS ORGANISED BY BUILD NUMBER,
SO SAY SO HERE RATHER THAN LEAVING IT TO BE INFERRED.** The board pagination work
(section 1, the `fetchOnline` offset row) is a change to `index.html`, which is
the half that carries `VERSION` and the `CHANGELOG`, so `VERSION` went 213 to 214
and CHANGELOG gained a 214 entry. That row is now CLOSED and its quoted marker no
longer exists in the file, so it can no longer be searched for -- the row names
the functions instead. Every OTHER row below was read at 213 and its line numbers
...
| --- | --- | --- | --- |
| `src/lib/legacy/vanguard/index.html:5501` | LEGACY BUILD + backend | `local earned title on the player's own row only (broadcast is a deferred follow-up)` -- the achievement title chip is drawn beside YOUR row on the leaderboard and nobody else's, because no other player's title ever arrives. | The board rows come from the Apps Script backend (`action=top`), so carrying a title means adding a field to what `submitToServer` sends and to what the sheet returns, then rendering it in `_achTitleChip`'s caller for rows other than `me`. Backend work outside this repo. | **STILL BLOCKED at build 214 (2026-09-05), and for a different reason than the pagination row above was.** That one was blocked on a QUESTION about the backend, which the supplied `Code.gs` answered; this one is blocked on backend WORK, which no answer removes. Carrying another player's title needs a new column in the Scores sheet, a field on `action=submit` and a field on what `action=top` returns, all of it in a deployment outside this repository. The client half cannot be built first: with nothing in the row to read, a chip rendered for a row other than `me` would be empty on every row forever. Mr. Pina owns this one.
