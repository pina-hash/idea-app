...
| --- | --- | --- | --- |
| `IDEA_instructions.md` | 4.17 | 2026-09-02 | Standing directive: doc-maintenance triggers, the freshness protocol for this directory (the fetch is a clone, not a raw URL), materials production pointer, Claude Code prompting and routing with Fable 5.1 as the default, the audit phase, the claims rule, `tools/idea-status.py` first, decisions owed, merge cadence and the Deploy button, derived numbers are generated, cloud-container environment facts, git/branch/migration rules, per-repo migration apply paths |
...
| `IDEA_Chat_Handoff_Standard.md` | 1.2 | 2026-09-02 | Governs every prompt written for a new claude.ai chat: when a handoff happens, the mandatory routing header, chat model routing (Fable 5.1; router chats always), kickoff content rules including decisions owed at the top, and closing a chat, where the closing chat writes the closeout kickoff prompt itself |
| `IDEA_REPO_WORKFLOW_STANDARD.md` | 1.0 | 2026-09-02 | One development workflow across `idea-app`, `fll-app` and `frc-app`: `idea-app` as the reference, what every repo carries (`CLAUDE.md` sections, the three workflows, `docs/history`, ledger, decisions, status tool, harnesses, tests, numbered migrations), what differs on purpose, and the dated conformance table |
