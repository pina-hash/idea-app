...
| --- | --- | --- | --- |
| `IDEA_instructions.md` | 4.21 | 2026-09-06 | Standing directive: doc-maintenance triggers, the freshness protocol for this directory (the fetch is a clone, not a raw URL), materials production pointer, Claude Code prompting and routing with Opus 5 as the default build tier and Fable 5.1 for what a session must decide on its own, the audit phase, the claims rule, `tools/idea-status.py` first, decisions owed, merge cadence and the Deploy button, derived numbers are generated, cloud-container environment facts, git/branch/migration rules, per-repo migration apply paths, the ledger status lifecycle and the rule that prompts in flight come from the ledger rather than the branch list, the canned lane ending that now lets a lane apply its own migration through `tools/apply-migration.mjs` and merge `integration` into `main` against a six-item reported checklist, and the migration-number rule that a number is allocated in the prompt and claimed in the session's first commit rather than read off the highest number on `origin/main` |
