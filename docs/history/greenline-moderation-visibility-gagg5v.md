...
106/53/83/212: +7 specs, +1 distinct route, +14 runs).

The measured region was then regenerated by a full `npm run verify:readme` pass on a clean tree
at commit `f43e589`, `dirty: false`: **226 route/width runs, 3300 measurements, 2 outside
threshold, 566.1s**, with the selftest reporting 70 controls / 36 negative / 34 positive / 0
failures in the same pass.

**Rows by identity, against A6.** `covered` was **103** and is now **113**, which is +7 for
this bundle's specs and +3 for the three `themes*` specs that A6 recorded as never measured --
so `covered` now equals the static `Route specs` figure exactly and `unmeasuredSpecs` is empty
for the first time in this file's history. `runsMeasured` 206 -> **226**. The **2 outside
threshold are not this bundle's**: both are the pre-existing `/dev/notebook` `tap-reach` rows
("toolbar text controls (under the floor on width -- decision 12, with the owner)"), carried
verbatim from the A6 baseline's own `outsideRows`. Every one of the 202 measurements on the
seven new specs is inside threshold.
...
`npm test`: **277 files, 5678 tests, all passing**, run at **13:19:46 PDT on 2026-09-05**
(America/Los_Angeles), 269.57s.

An earlier run at **13:02:02 PDT**, before B6's regeneration, failed 2 tests in
`tests/derived-numbers.test.ts` -- both of them that file correctly noticing that the measured
README region named the seven new specs as never measured. That is the test doing its job (a
stale measured region is exactly what it exists to catch), and it is resolved by the
`verify:readme` pass above rather than by touching the test. Recorded here because "the suite
was red once" is a fact a later reader should be able to see the shape of.
