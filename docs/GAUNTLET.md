...
| `0184` | **ADDED 2026-09-06.** Bounds what one run may write through `gauntlet_run_events_insert`. Its `anon` grant is correct and stays (a SolidWorks add-in and two VBA macros hold the run code and no session); what it was missing is any bound at all, measured against the deployed function as `anon`: 5,000 rows in one call, a 2 MB payload in one row, and a token 30 days expired still writing. Three bounds go inside the existing function with the signature unchanged, so there is no deploy ordering and no client change: refuse past the token's `expires_at`, cap rows per RUN (`_gauntlet_run_event_cap`, 20000), cap bytes per CALL (`_gauntlet_event_bytes_cap`, 262144). It still never raises -- an over-cap call is clipped and returns a short count, per `0035` section 4's best-effort contract. The window keys on `expires_at` ALONE and never on `locked_at`, because a correct solo submit sets `locked_at` and the add-in's final `run_end` flush posts after it. Cross-person forgery was already impossible and is asserted so; self-forgery stays open, which `0152` decided deliberately and this does not reopen |
| `0194` | **ADDED 2026-09-10.** The plausibility floor becomes a setting an admin owns (`gauntlet_rank_settings`, a singleton with `gauntlet_rank_settings_get` / `_set`, both admin only and the table readable by nobody else), and a run under it is HELD IN VIEW rather than dropped out of it: `gauntlet_leaderboard` gains `rank_state`, a held run sits on the board carrying `pending_verification` with a NULL `rank`, and the `distinct on` prefers a ranked row so a student holding both keeps their honest seat. `0154` pinned its literal to `0152`'s own threshold so that no run could lose a seat unreported; that apply-time assertion is REPLACED by two runtime properties -- the board never drops the run at all, and `gauntlet_run_review` emits a `pending_verification` observation computed from the SAME settings row, never from `p_fast_finish_seconds`, so a narrowed lens plus `p_observed_only` cannot hide one. Decision 19; `tests/db/gauntlet-verification-floor.test.ts` is the proof |
