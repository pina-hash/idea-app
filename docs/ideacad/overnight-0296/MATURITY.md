...
| Viewport and display | 1 | **3** | Front, Top, Right, Iso, Normal To (with flip), Fit framing the model clear of open panels, a corner triad that turns with the view, four display modes, size-relative display tessellation (a disk from 2,580 to 5,956 triangles, orbit p95 1.7 to 0.8 ms), right-drag orbit, refit when an edit runs off screen, 0 px overlap at 960. Open: F085 (arc spec canvas reading), F045 (seams), no capped section. |
