...
-- IDEA // GAUNTLET: ranked verification is GEOMETRY (volume) ONLY. Density and
-- target mass are ALWAYS sourced from the level record, NEVER from the part's
-- assigned material.
--
-- ROOT CAUSE this reverses: 0026 gated the macro submit on the part's assigned
-- MATERIAL NAME, then 0027 gated on the part's assigned MATERIAL DENSITY
-- (mass / volume), and 0030 additionally blocked on the DOCUMENT unit system.
-- All three read something off the student's part other than its geometry, so a
-- missing / custom / mismatched material (or an IPS document) hard-failed a
-- geometrically-correct part. This migration removes every such gate.
--
-- The model now:
--   * RANKED / leaderboard verification = measured geometric volume vs the
--     level's stored expected volume (answer->>'target_volume_mm3') within
--     tolerance (volume-as-checksum). It never reads or requires an assigned
--     material and never blocks on material or document units. This restores the
--     0016 volume-only path.
--   * MASS stays first-class but is COMPUTED FROM THE LEVEL'S DENSITY:
--     mass = measured_volume x level_density. Because density is a fixed level
--     constant, hitting target mass == hitting target volume, so ranked stays
--     volume-only and needs no second mass gate. The result reports both the
--     level's target mass and the student's computed mass, in the level's unit
--     system (TooTallToby: IPS -> lb, MMGS -> g).
--   * The applied material name (p_material) is retained ONLY as a non-gating
--     advisory (material_matches) for display; it never affects pass/fail. The
--     part's assigned-material mass (p_mass_g) is NOT read for any mass or
--     verification value.
--   * The document unit system (p_unit_system) is retained ONLY as a non-gating
--     informational field; the comparison never branches on it.
--
-- Tolerance: the relative-tolerance constant is GAUNTLET_VOLUME_TOL_PCT = 0.5
-- (percent). It is the shared default across the server, the VBA macros, and the
-- C# add-in; keep them in sync. A level may still override via
-- answer->>'tolerance_pct'.
--
-- Volume basis: p_volume_mm3 is CANONICAL cubic millimetres, extracted by the
-- client on an explicit SI basis (mass properties UseSystemUnits = true -> m^3)
-- and converted once via 1 m^3 = 1e9 mm^3. No layer branches on display units to
-- read volume. The level's stored target_volume_mm3 and density are in that same
-- canonical basis (density in the level's unit; IPS lb/in^3 is normalized to
-- g/cm^3 here with 1 lb/in^3 = 27.679904653 g/cm^3).
--
-- Signature is unchanged from 0030 (8-arg), so existing macros / add-in builds
-- keep working; the extra params are simply no longer gates. Apply manually in
-- the Supabase SQL editor. Idempotent.

-- ---------------------------------------------------------------------------
-- 1. gauntlet_macro_submit: volume-only ranked, mass from level density.
-- ---------------------------------------------------------------------------
...
declare
	...
	c_volume_tol_pct constant numeric := 0.5;
...
-- ---------------------------------------------------------------------------
-- 2. gauntlet_run_targets: read the level constants for a valid code, so the
--    macro / add-in can show target vs computed mass (from LEVEL density) and
--    run the unranked practice mass-verify without reading the part's material.
--    SECURITY DEFINER, anon-granted (the code is the credential; these values
--    are already shown on the web run screen after reveal). Read-only: it never
--    writes a submission and never affects ranked state.
-- ---------------------------------------------------------------------------
...
declare
	...
	v_mass_unit text;
	v_length_unit text;
	c_volume_tol_pct constant numeric := 0.5;   -- keep in sync (see gauntlet_macro_submit)
	c_lb_in3_to_g_cm3 constant numeric := 27.679904653;
	c_lb_to_g constant numeric := 453.59237;
begin
	if p_code is null or length(trim(p_code)) = 0 then
...
-- ---------------------------------------------------------------------------
-- 3. Publish blocker: MATERIAL is no longer required to publish a modeling
--    challenge (it is only a non-gating advisory label now). Density stays
--    required, because mass is computed from it. Everything else is verbatim
--    from 0009. (A stored material still works; it is just optional.)
-- ---------------------------------------------------------------------------
...
begin
	if p_mode in ('speedrun', 'reverse_engineer', 'feature_golf') then
		...
		if coalesce(public.gauntlet_jnum(p_answer, 'density'), 0) <= 0 then
			return 'Density is required to publish (mass is computed from it).';
		end if;
		-- Material is optional now: ranked verification is volume-only and mass
		-- uses the level density, so a material name is a display advisory only.
