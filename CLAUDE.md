...
- **Schema:** `notebook_sections` (**DROPPED IN `0094` -- sections and rosters
  ...
  `notebook_sessions` (required check-ins: unit, date, label; `section_id`
  references `classroom_sections` since `0094`),
...
- **The composite FK is preserved exactly.** `notebook_entries (session_id,
  ...
  `23503`). There is deliberately no equivalent reaching into
