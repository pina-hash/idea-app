...
### The Ledger keeps working, unedited
...
the four-level badge (OFF / COACH / ASSIST / OPEN), the two-part Declaration
(integrity checkbox + AI disclosure) every assignment carries, and the
per-category defaults. **When building a new assignment, the AI-level for
each module is set by asking the instructor, never inferred from the content
alone.** The category-defaults table in `IDEA_AI_Use_Policy.md` ("Category
Defaults") is the starting point for that conversation, not a rule that
self-applies.
...
### THE PLATFORM CEILING, flagged because it is not ours to raise

The caps in `classroom-decks.ts` are about memory and zip bombs (48 MB zip / 96
MB unpacked / 500 files). **On Vercel the binding limit is lower: a serverless
function's request body is capped at ~4.5 MB**, so a deck zip past that is
refused by the PLATFORM before any of this code runs. The `(standalone).html`
variant an export ships is usually most of the weight and is skipped anyway, so
re-zipping without it is the practical answer -- but a genuinely large deck
(many `uploads/` gifs) will need a different upload path (chunked, or
direct-to-Drive) that this pass does not build.
...
- **Reached from the class it belongs to, and summarized where a class is
  ...
  `/classroom/manage`'s per-section panel shows a compliance SUMMARY built from
...
- **Routes** (`/classroom` is in `authedPrefixes`; anonymous 303 to `/`,
  live-verified on all three depths): `/classroom` (My Classes -- ONE
  RLS-scoped sections select serves students and teachers alike, no role
  branch in the query); `/classroom/[sectionId]` (Stream = posts +
...
- **ONE content editor, mounted everywhere content is seen.**
  ...
  class STREAM and CLASSWORK cards, and the assignment detail page. Edit and
...
- **The tab rail's active state** is a filled tab (`--bg1`), green 700-weight
  label, a 3px green cap and a break in the rail's own underline, against dim
  400-weight text on transparent. The old version differed only in text colour.
