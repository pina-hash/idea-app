<script lang="ts">
	...
	import { execUrl, fetchSelection, type FspSelection } from '$lib/fsp-pulse/client';

	/**
	 * /fsp-pulse — FSP Pulse Check, a non-binding interest pulse (pathway
	 * ranking + FRC interest) for incoming freshmen during FSP, reached COLD
	 * from a QR code. Reuses the existing Google OAuth flow, restricted to
	 * @boscotech.net. Writes DIRECTLY to its own Apps Script endpoint (no
	 * Supabase). Neutral Bosco Tech branding, scoped under `.fsp-root`.
	 *
	 * This is a SEPARATE tool from the earlier /fsp-tech-selection proposal
	 * (which was rejected as a selection mechanism). This tool has no bearing
	 * on official pathway assignment; see the in-component banner.
	 *
	...
	async function signIn() {
		...
		const { error } = await supabase.auth.signInWithOAuth({
			provider: 'google',
			options: {
				redirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent('/fsp-pulse')}`,
...
<svelte:head>
	<title>Bosco Tech — FSP Pulse Check</title>
...
<div class="fsp-root">
	...
	{#if !signedIn}
		<div class="gate">
			<div class="gate-card">
				...
				<p>
					Quick pulse check for incoming freshmen. Takes under a minute, update it any time before
					FSP wraps up. Sign in with your Bosco Tech account to begin.
				</p>
				...
				<a class="staff-link" href="/fsp-pulse/preview">See how staff view this &rarr;</a>
			</div>
		</div>
	{:else if !domainOk}
		<div class="gate">
	...
	{:else}
		<FspPulse {email} {initial} execUrl={url} />
	{/if}
</div>
