 ...
 * (`s.mode in ('speedrun')`) that reverse_engineer is deliberately kept off,
 * because its returned score_metric is an exact-deviation oracle on its own
 * target. Board presence therefore reads as `cleared: false` for EVERY level,
 * however many a student has genuinely cleared. 0154 then added the 30s
 * plausibility floor to that same branch, so the speedrun list stopped trusting
 * board presence for `cleared` too; this file carries the same fix for the same
 * reason. bestMetric and rank still come from the board and are null for this
 * mode today, which is correct and was already true.
 */
