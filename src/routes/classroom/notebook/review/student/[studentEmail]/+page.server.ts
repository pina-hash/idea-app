 ...
 *
 * INSIDE THE CLASSROOM NOW (ledger 0297, package F4a). This was
 * `/notebook/review/student/<email>`, which redirects here with its query
 * string; the load below is that route's, unchanged, plus the one thing a page
 * inside the classroom shell owes: its way back, as crumbs.
 *
...
export const load: PageServerLoad = async ({ params, url, locals: { supabase, claims } }) => {
	...
	const [access, { data, error: rpcError }] = await Promise.all([
		...
		supabase.rpc('notebook_review_student_notebook', {
	...
	/**
	 * THE WAY BACK, AS CRUMBS. A manager of the class this was opened from goes
	 * back to that class's own Notebook tab, which IS its grid; anybody else --
	 * a 0169 reviewer, who cannot open the class as a class -- goes back to the
	 * all-sections console on that section. `classroom_manages_section` is the
	 * same question the class's tab asks, and the section read is RLS-scoped, so
	 * neither can name a class the viewer could not already open.
	 */
	let returnTo: Crumb[] = [{ label: 'Notebook review', href: notebookReviewHref(fromSectionId) }];
	if (fromSectionId) {
		const [{ data: manages }, { data: sectionRow }] = await Promise.all([
			supabase.rpc('classroom_manages_section', { p_section_id: fromSectionId }),
			supabase
		...
		if (manages === true && sectionRow) {
			const section = normalizeSectionRow(sectionRow as Record<string, unknown>);
			returnTo = [
				{ label: sectionTitle(section), href: `/classroom/${section.id}` },
				{ label: 'Notebook', href: classNotebookHref(section.id) }
			];
		}
	}
	...
	return {
		 ...
		 * (`notebook_staff_restore_entry`) gates on manage-ness and is deliberately
		 * NOT widened to the 0169 reviewer tier -- restore is deletion's undo and
		 * both are moderation, not review -- so a reviewer-only viewer gets no
		 * control whose every press would be refused. Coarse on purpose (chair or
		 * teacher-of-record-of-anything, the whole pre-0169 population of this
		 * page): the payload carries no section ids to ask a per-student question
		 * with, and the RPC's own gate is the boundary either way.
		 */
		canRestore: access.isChair || access.isInstructor,
		student: payload.student,
		fromSectionId,
		/** The crumbs between My Classes and this student, read by the classroom layout. */
		notebookReturnTo: returnTo,
		/** Where the back strip goes: the last of those crumbs. */
		backHref: returnTo[returnTo.length - 1].href ?? notebookReviewHref(),
		/**
