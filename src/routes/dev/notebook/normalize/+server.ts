...
/**
 * Dev-only: runs the REAL note normalizer (the sanitizer) for /dev/notebook.
 *
 ...
 * what the harness stores and renders is what the real routes would have
...
export const POST: RequestHandler = async ({ request }) => {
	...
	const result = normalizeNoteDoc(body.content);
	return result.ok
		? json({ ok: true, doc: result.doc })
