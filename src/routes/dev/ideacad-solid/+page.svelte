...
<main>{#if chosen}<SolidWorkspace {document} {transport} {advisoryTransport} dev onback={()=>chosen=false}/>{:else}<section class="chooser"><h1>IdeaCAD</h1><button disabled={!ready} onclick={()=>chosen=true}>+ New document</button>{#if document.revision}<button onclick={()=>chosen=true}>{document.title}</button>{/if}</section>{/if}</main>
