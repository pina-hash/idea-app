...
/**
 * The student-facing digital notebook's LOAD, one implementation for both of
 * the routes that mount it (ledger 0297, package F4a): the whole notebook at
 * `/classroom/notebook` and a class's own Notebook tab at
 * `/classroom/<id>/notebook`. It moved here, unchanged, from
 * `src/routes/notebook/+page.server.ts`, which is a redirect into the classroom
 * now -- the notebook lives inside the class it belongs to.
 *
 ...
 * turned away by hooks.server.ts (the '/classroom' authed prefix); each route
 * that calls this redirects a missing session itself, belt-and-braces.
 *
 ...
 */
export async function loadStudentNotebook({
	url,
	supabase,
	claims,
	sectionId = null
	...
	/**
	 * THE CLASS THIS NOTEBOOK IS SCOPED TO, or null for the student's whole
	 * notebook. Set by a class's own Notebook tab (`/classroom/<id>/notebook`):
	 * the entries are the student's own entries FILED TO THIS CLASS and the
	 * check-ins are this class's. See the three places it narrows below.
	 */
	...
	/**
	 * THE STUDENT'S CLASSES, for the class a free entry is filed to (ledger
	 * 0297). A "Something else" entry used to be saved with no class at all, so
	 * free work belonged to nobody and the class grid's free-entry counter never
	 * saw it. The whole notebook offers these as a picker; a class's own tab
	 * needs none, because the class IS the answer.
	 */
	...
	if (configured) {
		...
		/**
		 * A CLASS'S TAB READS ONE CLASS. The select above is unchanged -- RLS
		 * already returns exactly the classes the caller is in -- and the scope
		 * is a filter over its answer, so a tab for a class the caller cannot
		 * read comes back with nothing rather than with a class they asked for.
		 * (The section layout above the tab has already 404'd such a caller.)
		 */
		const rows = ((sectionRows ?? []) as SectionRow[]).filter(
			(r) => !sectionId || r.id === sectionId
		);
		...
		// A class's tab carries no chip: the class is already the crumb above it.
		...
		classes = rows
			.map((r, i) => ({ id: r.id, label: names[i] || r.label }))
	...
	if (configured && deletionReady) {
		const deletedRead = supabase
			.from('notebook_entries')
			.select(NOTEBOOK_DELETED_SELECT)
			.eq('student_id', claims.sub)
			.not('deleted_at', 'is', null)
			.order('deleted_at', { ascending: false });
		// A class's tab offers back only what was deleted FROM this class.
		const { data: deletedRows, error: deletedError } = sectionId
			? await deletedRead.eq('section_id', sectionId)
			: await deletedRead;
		if (!deletedError) {
	...
	const askedSection = url.searchParams.get('section') ?? sectionId;
	...
	/**
	 * THE CLASS A FREE ENTRY IS FILED TO BY DEFAULT: the class the student came
	 * from (`?section=`, validated against their own classes exactly as the
	 * check-in above is, so it can only pick something the page was going to
	 * offer), else their only class. A class's own tab is scoped to its class
	 * and asks nothing.
	 */
	const defaultSectionId = sectionId
		? sectionId
		: (classes.find((c) => c.id === askedSection)?.id ??
	...
	return {
		...
		/** The class this notebook is scoped to (a class's own tab), or null. */
