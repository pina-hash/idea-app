...
<section class="card contracts-manager">
	...
	{:else}
		<div class="sub-panel">
			...
			<div class="field-row">
				<label for="contract-payout">Payout (i&cent;, split evenly across whoever claims it)</label>
			...
			<div class="guideline">
				<p class="note small">
					Guideline only, never enforced: ~1i&cent;/hour, +50% for specialized skill over general
				...
				<div class="guideline-row">
					...
					{#if guidelinePreview !== null}
						<span class="preview">suggests {guidelinePreview}i&cent;</span>
					{/if}
				</div>
			</div>
										...
										<span class="claimant-chip">
											...
											{#if c.status === 'open' || c.status === 'full'}
												<span class="preview-inline">
													&rarr; {contractSharePreview(c.payout_amount, c.claimed_count)}i&cent; if completed now
												</span>
											{/if}
										</span>
									{/each}
