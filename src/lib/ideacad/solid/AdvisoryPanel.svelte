...
<style>
	.revision{font:12px 'Share Tech Mono',monospace;color:var(--text-2);padding:12px 0}.check{border-top:1px solid var(--boundary);padding:10px 0;display:grid;gap:4px}.check>div{display:flex;justify-content:space-between;gap:8px;align-items:center}strong{font-size:17px}.check span{font-size:12px}.pass{color:var(--green)}.fail{color:var(--warning)}.unknown,small{color:var(--text-2)}output{font:17px 'Share Tech Mono',monospace}small{font-size:14px}dl{font-size:14px}dt{margin-top:8px;color:var(--text-2)}dd{margin:2px 0}button{min-height:44px;width:100%;border:1px solid var(--green);border-radius:5px;background:var(--surface-0);color:var(--green);font:600 17px Rajdhani,sans-serif;cursor:pointer}
</style>
