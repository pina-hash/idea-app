...
export default {
	...
	label: 'The pending primitive in the notebook room under IDEA (the default theme) (.nb-root in .cr-root)',
	widths: WIDTHS,
	/*
		...
		readings are three routes: this one, and its siblings under matrix and space-white.
		`contrast` reports the worst match, and with one room per page nothing
		collapses two grounds into one number.
	*/
	presence: [
		{ selector: '[data-testid="nb-room"].nb-root[data-site="idea"]', label: 'the notebook room, under IDEA (the default theme)', expectPresent: 1, maxPresent: 1, expectVisible: 1, maxVisible: 1 },
	...
	contrast: [
		{ selector: '.nb-root .card .pending', label: 'pending label, IDEA (the default theme)', min: 4.5 }
	]
};
