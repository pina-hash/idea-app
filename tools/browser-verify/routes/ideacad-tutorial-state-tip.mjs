 ...
 * card beside the tool rather than over it (the `overlaps` check is the
 * placement's own test in `tests/ideacad-solid-learn.test.ts`; here it is
 * seen at each width on the real page).
 */
export default {
	...
	prepare: [
		...
		{ waitFor: '() => { const t = document.querySelector(\'[data-command="extrude"] ~ .tip\'); if (!t || t.hidden || getComputedStyle(t).visibility !== "visible") return false; const a = document.querySelector(\'[data-command="extrude"]\').getBoundingClientRect(), b = t.getBoundingClientRect(); return b.right <= a.left || b.left >= a.right || b.bottom <= a.top || b.top >= a.bottom; }', timeoutMs: 5000 }
	],
	presence: [
		{ selector: '[data-command="extrude"] ~ .tip:not([hidden])', label: 'the Extrude card, open and clear of its tool', expectPresent: 1, maxPresent: 1, expectVisible: 1 },
