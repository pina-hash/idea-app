/**
 * CAPTURE WHERE THE WORK IS (ledger 0297, package F4b): a student on an item
...
export default {
	path: '/dev/notebook-capture?latency=150',
	label: 'Item page notebook capture, student (two photos uploaded as a draft on take)',
	prepare: [
	...
	presence: [
		...
		{ selector: '[data-testid="item-check-in-link"]', label: 'the old notebook link (must be absent: capture replaces it)', expectPresent: 0 },
	...
	orderResult: [
		{
			...
			expected: [
				'first call: upload page1.png section_id,session_id,submitted | submitted=false',
