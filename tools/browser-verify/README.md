...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 155 |
...
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 310 |

<!-- counts:static:data {"schema":1,"specs":155,"routes":67,"devPages":95,"widths":[375,1440],"runs":310} -->
<!-- counts:static:end -->

### Measured -- from a full harness run

<!-- counts:measured:begin -->
**Generated by `npm run verify:readme`; do not edit by hand.** Measured 2026-09-09T13:01:03.801Z on commit `c50a322` (working tree dirty at measurement) by a full run of `tools/browser-verify/run.mjs` in this container. It needs a browser and about six minutes, so it is regenerated deliberately and not on every branch; a stale-but-honest measured half is a supported state.
...
| Count | Value |
| --- | --- |
| Route specs the run covered | 155 |
| Route/width runs the report carried | 310 |
| Measurements | 5180 |
| Measurements outside threshold | 2 |
| Full-run wall clock | 745.3s |
