...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 433 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 127 |
| Directories under `src/routes/dev` with a page (the candidate set) | 155 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 866 |

<!-- counts:static:data {"schema":1,"specs":433,"routes":127,"devPages":155,"widths":[375,1440],"runs":866} -->
<!-- counts:static:end -->
...
| Count | Value |
| --- | --- |
| Route specs measured (one file each under `measured/`) | 433 |
| Route/width runs those measurements cover | 866 |
| Measurements | 14624 |
| Measurements outside threshold | 211 |
| Wall clock, summed from each spec's attributed share | 2786.8s |
