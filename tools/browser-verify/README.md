...
by hand about a number the harness computes on every run. `npm run
verify:readme` runs the harness and rewrites the block between the markers;
`npm run verify:readme -- --check` exits non-zero when the committed block
disagrees with a fresh run; `tests/derived-numbers.test.ts` fails CI when a
value in it was edited by hand or a spec was added without regenerating.
`readme-counts.mjs` is the only writer. Quote the block, never a number from
prose, and if prose elsewhere in this file carries a count, the block wins.

<!-- counts:begin -->
**Generated by `npm run verify:readme`; do not edit by hand.** Measured 2026-09-02T15:49:06.038Z on commit `1c9fa2c` by a full run of `tools/browser-verify/run.mjs` in this container. `tests/derived-numbers.test.ts` reddens on a hand edit or on a spec added without regenerating.

| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 68 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 38 |
| Directories under `src/routes/dev` with a page (the candidate set) | 70 |
| Widths | 2 (375, 1440) |
| Route/width runs (specs x widths) | 136 |
| Route/width runs the report carried | 136 |
| Measurements | 1772 |
| Measurements outside threshold | 8 |
| Full-run wall clock | 308.8s |
