...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 142 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 66 |
| Directories under `src/routes/dev` with a page (the candidate set) | 94 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 284 |

<!-- counts:static:data {"schema":1,"specs":142,"routes":66,"devPages":94,"widths":[375,1440],"runs":284} -->
<!-- counts:static:end -->

### Measured -- from a full harness run

<!-- counts:measured:begin -->
**Generated by `npm run verify:readme`; do not edit by hand.** Measured 2026-09-06T23:40:56.818Z on commit `287f079` by a full run of `tools/browser-verify/run.mjs` in this container. It needs a browser and about six minutes, so it is regenerated deliberately and not on every branch; a stale-but-honest measured half is a supported state.
...
| Count | Value |
| --- | --- |
| Route specs the run covered | 140 |
| Route/width runs the report carried | 280 |
| Measurements | 4296 |
| Measurements outside threshold | 2 |
| Full-run wall clock | 674.1s |
