...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 287 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 103 |
| Directories under `src/routes/dev` with a page (the candidate set) | 129 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 574 |

<!-- counts:static:data {"schema":1,"specs":287,"routes":103,"devPages":129,"widths":[375,1440],"runs":574} -->
<!-- counts:static:end -->

### Measured -- summed from `measured/`, one file per route spec

<!-- counts:measured:begin -->
**Generated by `npm run verify:counts` from `measured/`; do not edit by hand.** Every route spec carries its OWN measurement file there, written by `npm run verify:readme` -- which needs a browser and about seventeen minutes for the whole tree, and seconds for one spec with `-- --route <spec>`. This table is the sum over those files, so it is a tree read like the static region above and costs nothing to regenerate on a merged tree. The newest measurement here was taken on commit `4cd231e`; measurements were taken at 2026-09-23T13:57:15.144Z. A stale-but-honest measured half is a supported state.
...
| Count | Value |
| --- | --- |
| Route specs measured (one file each under `measured/`) | 287 |
| Route/width runs those measurements cover | 574 |
| Measurements | 10298 |
| Measurements outside threshold | 228 |
| Wall clock, summed from each spec's attributed share | 1613.5s |
| `--selftest` controls | 100 (51 negative, 49 positive), 0 instrument failure(s) |
...
- `/dev/grading-incomplete` @375 `prepare-click` [data-testid="work-export-disclosure"]
- `/dev/grading-incomplete` @375 `tap-target` JSON: this student
- `/dev/grading-incomplete` @375 `tap-target` JSON: whole class
- `/dev/grading-incomplete` @375 `tap-target` Spreadsheet: whole class
- `/dev/grading-incomplete` @375 `tap-target` the identity switch, measured at its label
- `/dev/grading-incomplete` @375 `presence` the graded-work export group
- `/dev/grading-incomplete` @375 `presence` JSON: this student
- `/dev/grading-incomplete` @375 `presence` JSON: whole class
- `/dev/grading-incomplete` @375 `presence` Spreadsheet: whole class
- `/dev/grading-incomplete` @375 `presence` the identity switch
- `/dev/grading-incomplete` @375 `presence` what the switch is currently doing, in words
...
- `/dev/ideacad-solid?state=arc` @375 `canvas-content` the viewport with the drawn arc
- `/dev/ideacad-solid?state=arc` @1440 `canvas-content` the viewport with the drawn arc
...
- `/dev/themes?state=matrix` @375 `presence` launcher cards
- `/dev/themes?state=matrix` @1440 `presence` launcher cards
- `/dev/themes` @375 `presence` launcher cards
- `/dev/themes` @1440 `presence` launcher cards
