...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 175 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 70 |
| Directories under `src/routes/dev` with a page (the candidate set) | 97 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 350 |

<!-- counts:static:data {"schema":1,"specs":175,"routes":70,"devPages":97,"widths":[375,1440],"runs":350} -->
<!-- counts:static:end -->

### Measured -- from a full harness run

<!-- counts:measured:begin -->
**Generated by `npm run verify:readme`; do not edit by hand.** Measured 2026-09-10T03:12:42.305Z on commit `9747543` by a full run of `tools/browser-verify/run.mjs` in this container. It needs a browser and about six minutes, so it is regenerated deliberately and not on every branch; a stale-but-honest measured half is a supported state.
