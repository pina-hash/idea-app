...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 116 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 57 |
| Directories under `src/routes/dev` with a page (the candidate set) | 86 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 232 |

<!-- counts:static:data {"schema":1,"specs":116,"routes":57,"devPages":86,"widths":[375,1440],"runs":232} -->
<!-- counts:static:end -->

### Measured -- from a full harness run

<!-- counts:measured:begin -->
**Generated by `npm run verify:readme`; do not edit by hand.** Measured 2026-09-06T07:36:43.955Z on commit `5d7cf5c` by a full run of `tools/browser-verify/run.mjs` in this container. It needs a browser and about six minutes, so it is regenerated deliberately and not on every branch; a stale-but-honest measured half is a supported state.
...
| Count | Value |
| --- | --- |
| Route specs the run covered | 116 |
| Route/width runs the report carried | 232 |
| Measurements | 3410 |
| Measurements outside threshold | 2 |
| Full-run wall clock | 564.7s |
