...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 214 |
...
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 428 |

<!-- counts:static:data {"schema":1,"specs":214,"routes":85,"devPages":113,"widths":[375,1440],"runs":428} -->
<!-- counts:static:end -->

### Measured -- summed from `measured/`, one file per route spec

<!-- counts:measured:begin -->
**Generated by `npm run verify:counts` from `measured/`; do not edit by hand.** Every route spec carries its OWN measurement file there, written by `npm run verify:readme` -- which needs a browser and about seventeen minutes for the whole tree, and seconds for one spec with `-- --route <spec>`. This table is the sum over those files, so it is a tree read like the static region above and costs nothing to regenerate on a merged tree. The newest measurement here was taken on commit `f86e535` (a working tree was dirty at measurement); measurements were taken between 2026-09-12T05:18:26.073Z and 2026-09-13T05:32:19.971Z. A stale-but-honest measured half is a supported state.
...
| Count | Value |
| --- | --- |
| Route specs measured (one file each under `measured/`) | 214 |
| Route/width runs those measurements cover | 428 |
| Measurements | 7650 |
| Measurements outside threshold | 0 |
| Wall clock, summed from each spec's attributed share | 1169.2s |
