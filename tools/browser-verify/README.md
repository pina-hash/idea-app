...
| Full-run wall clock | 379.4s |
| `--selftest` controls | 64 (32 negative, 32 positive), 0 instrument failure(s) |

Measurements outside threshold on that run:

- `/dev/pathways` @375 `tap-target` ProfileMenu trigger (ships in every page header)
- `/dev/pathways` @1440 `tap-target` ProfileMenu trigger (ships in every page header)
- `/dev/coins-signedin-1` @375 `horizontal-scroll`
- `/dev/coins` @375 `prepare-eval` () => { const d = document.documentElement; const over = d.scrollWidth
- `/dev/coins` @375 `horizontal-scroll`
- `/dev/coins` @1440 `prepare-eval` () => { const d = document.documentElement; const over = d.scrollWidth

<!-- counts:measured:data {"schema":1,"date":"2026-09-03T22:28:05.043Z","sha":"ffcbf20a641df1962aefe64b0c07904d82ce2557","dirty":false,"runsMeasured":160,"measurements":2202,"outside":6,"outsideRows":[{"path":"/dev/pathways","width":375,"check":"tap-target","label":"ProfileMenu trigger (ships in every page header)"},{"path":"/dev/pathways","width":1440,"check":"tap-target","label":"ProfileMenu trigger (ships in every page header)"},{"path":"/dev/coins-signedin-1","width":375,"check":"horizontal-scroll","label":""},{"path":"/dev/coins","width":375,"check":"prepare-eval","label":"() => { const d = document.documentElement; const over = d.scrollWidth"},{"path":"/dev/coins","width":375,"check":"horizontal-scroll","label":""},{"path":"/dev/coins","width":1440,"check":"prepare-eval","label":"() => { const d = document.documentElement; const over = d.scrollWidth"}],"totalMs":379441,"selftest":{"controls":64,"negative":32,"positive":32,"failures":0}} -->
<!-- counts:measured:end -->
