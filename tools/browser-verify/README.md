...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 163 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 68 |
| Directories under `src/routes/dev` with a page (the candidate set) | 96 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 326 |

<!-- counts:static:data {"schema":1,"specs":163,"routes":68,"devPages":96,"widths":[375,1440],"runs":326} -->
<!-- counts:static:end -->

### Measured -- from a full harness run

<!-- counts:measured:begin -->
**Generated by `npm run verify:readme`; do not edit by hand.** Measured 2026-09-09T22:31:01.810Z on commit `85c7ea9` by a full run of `tools/browser-verify/run.mjs` in this container. It needs a browser and about six minutes, so it is regenerated deliberately and not on every branch; a stale-but-honest measured half is a supported state.
...
| Count | Value |
| --- | --- |
| Route specs the run covered | 163 |
| Route/width runs the report carried | 326 |
| Measurements | 5520 |
| Measurements outside threshold | 4 |
| Full-run wall clock | 833.1s |
