...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 209 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 84 |
| Directories under `src/routes/dev` with a page (the candidate set) | 112 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 418 |

<!-- counts:static:data {"schema":1,"specs":209,"routes":84,"devPages":112,"widths":[375,1440],"runs":418} -->
<!-- counts:static:end -->
...
| Count | Value |
| --- | --- |
| Route specs measured (one file each under `measured/`) | 209 |
| Route/width runs those measurements cover | 418 |
| Measurements | 7534 |
| Measurements outside threshold | 0 |
| Wall clock, summed from each spec's attributed share | 1160.1s |
