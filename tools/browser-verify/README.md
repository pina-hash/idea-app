...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 227 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 87 |
| Directories under `src/routes/dev` with a page (the candidate set) | 115 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 454 |

<!-- counts:static:data {"schema":1,"specs":227,"routes":87,"devPages":115,"widths":[375,1440],"runs":454} -->
<!-- counts:static:end -->
...
| Count | Value |
| --- | --- |
| Route specs measured (one file each under `measured/`) | 227 |
| Route/width runs those measurements cover | 454 |
| Measurements | 8056 |
| Measurements outside threshold | 0 |
| Wall clock, summed from each spec's attributed share | 1285.9s |
