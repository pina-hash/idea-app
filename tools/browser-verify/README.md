...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 158 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 67 |
| Directories under `src/routes/dev` with a page (the candidate set) | 95 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 316 |

<!-- counts:static:data {"schema":1,"specs":158,"routes":67,"devPages":95,"widths":[375,1440],"runs":316} -->
<!-- counts:static:end -->

### Measured -- from a full harness run

<!-- counts:measured:begin -->
**Generated by `npm run verify:readme`; do not edit by hand.** Measured 2026-09-09T22:22:53.311Z on commit `2dcb31f` by a full run of `tools/browser-verify/run.mjs` in this container. It needs a browser and about six minutes, so it is regenerated deliberately and not on every branch; a stale-but-honest measured half is a supported state.
