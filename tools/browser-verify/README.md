...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 194 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 79 |
| Directories under `src/routes/dev` with a page (the candidate set) | 107 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 388 |

<!-- counts:static:data {"schema":1,"specs":194,"routes":79,"devPages":107,"widths":[375,1440],"runs":388} -->
<!-- counts:static:end -->

### Measured -- from a full harness run

<!-- counts:measured:begin -->
**Generated by `npm run verify:counts` from `measured/`; do not edit by hand.** Every route spec carries its OWN measurement file there, written by `npm run verify:readme` -- which needs a browser and about seventeen minutes for the whole tree, and seconds for one spec with `-- --route <spec>`. This table is the sum over those files, so it is a tree read like the static region above and costs nothing to regenerate on a merged tree. The newest measurement here was taken on commit `949480c`; measurements were taken at 2026-09-12T05:18:26.073Z. A stale-but-honest measured half is a supported state.
...
| Count | Value |
| --- | --- |
| Route specs measured (one file each under `measured/`) | 194 |
| Route/width runs those measurements cover | 388 |
| Measurements | 6790 |
| Measurements outside threshold | 0 |
| Wall clock, summed from each spec's attributed share | 1032.7s |
