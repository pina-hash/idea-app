...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 303 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 106 |
| Directories under `src/routes/dev` with a page (the candidate set) | 135 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 606 |

<!-- counts:static:data {"schema":1,"specs":303,"routes":106,"devPages":135,"widths":[375,1440],"runs":606} -->
<!-- counts:static:end -->

### Measured -- summed from `measured/`, one file per route spec

<!-- counts:measured:begin -->
**Generated by `npm run verify:counts` from `measured/`; do not edit by hand.** Every route spec carries its OWN measurement file there, written by `npm run verify:readme` -- which needs a browser and about seventeen minutes for the whole tree, and seconds for one spec with `-- --route <spec>`. This table is the sum over those files, so it is a tree read like the static region above and costs nothing to regenerate on a merged tree. The newest measurement here was taken on commit `165f36d`; measurements were taken between 2026-09-23T13:57:15.144Z and 2026-09-23T14:16:01.519Z. A stale-but-honest measured half is a supported state.
...
| Measurements | 10298 |
| Measurements outside threshold | 226 |
| Wall clock, summed from each spec's attributed share | 1633.6s |
