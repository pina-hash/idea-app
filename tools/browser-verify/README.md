...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 120 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 59 |
| Directories under `src/routes/dev` with a page (the candidate set) | 88 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 240 |

<!-- counts:static:data {"schema":1,"specs":120,"routes":59,"devPages":88,"widths":[375,1440],"runs":240} -->
<!-- counts:static:end -->

### Measured -- from a full harness run

<!-- counts:measured:begin -->
**Generated by `npm run verify:readme`; do not edit by hand.** Measured 2026-09-06T08:09:22.598Z on commit `f4c30ad` by a full run of `tools/browser-verify/run.mjs` in this container. It needs a browser and about six minutes, so it is regenerated deliberately and not on every branch; a stale-but-honest measured half is a supported state.
