...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 230 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 88 |
| Directories under `src/routes/dev` with a page (the candidate set) | 116 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 460 |

<!-- counts:static:data {"schema":1,"specs":230,"routes":88,"devPages":116,"widths":[375,1440],"runs":460} -->
<!-- counts:static:end -->

### Measured -- summed from `measured/`, one file per route spec

<!-- counts:measured:begin -->
**Generated by `npm run verify:counts` from `measured/`; do not edit by hand.** Every route spec carries its OWN measurement file there, written by `npm run verify:readme` -- which needs a browser and about seventeen minutes for the whole tree, and seconds for one spec with `-- --route <spec>`. This table is the sum over those files, so it is a tree read like the static region above and costs nothing to regenerate on a merged tree. The newest measurement here was taken on commit `5399ccf` (a working tree was dirty at measurement); measurements were taken between 2026-09-12T05:18:26.073Z and 2026-09-14T06:37:41.546Z. A stale-but-honest measured half is a supported state.
...
| Count | Value |
| --- | --- |
| Route specs measured (one file each under `measured/`) | 230 |
| Route/width runs those measurements cover | 460 |
| Measurements | 8312 |
| Measurements outside threshold | 147 |
| Wall clock, summed from each spec's attributed share | 1289.8s |
| `--selftest` controls | 98 (50 negative, 48 positive), 0 instrument failure(s) |
...
- `/dev/ideacad?role=student&state=compare` @375 `tap-target` the concept picker
- `/dev/ideacad?role=student&state=compare` @375 `tap-target` the say-why field
- `/dev/ideacad?role=student&state=compare` @375 `tap-target` a compare-sheet control
- `/dev/ideacad?role=student&state=compare` @375 `distinguishable` a readout label against its own value
- `/dev/ideacad?role=student&state=compare` @1440 `tap-target` the concept picker
- `/dev/ideacad?role=student&state=compare` @1440 `tap-target` the say-why field
- `/dev/ideacad?role=student&state=compare` @1440 `tap-target` a compare-sheet control
...
- `/dev/ideacad?role=student&state=history` @375 `tap-target` the History toggle in the header
...
- `/dev/ideacad?role=student&state=history` @1440 `tap-target` Undo and Redo inside the panel
- `/dev/ideacad?role=student&state=history` @1440 `tap-target` the way back to the feature tree
- `/dev/ideacad?role=student&state=history` @1440 `tap-target` the History toggle in the header
- `/dev/ideacad?role=student&state=history` @1440 `layout-sanity` the IdeaCAD console
- `/dev/ideacad?role=student&state=materials` @375 `prepare-wait` () => !!document.querySelector("[data-testid=\"ideacad-mater
- `/dev/ideacad?role=student&state=materials` @375 `contrast` the panel heading on the pane ground
- `/dev/ideacad?role=student&state=materials` @375 `contrast` a control label
- `/dev/ideacad?role=student&state=materials` @375 `contrast` the density readout beside a picker
- `/dev/ideacad?role=student&state=materials` @375 `contrast` the panel’s own prose
- `/dev/ideacad?role=student&state=materials` @375 `contrast` the UNVERIFIED chip beside a density
...
- `/dev/ideacad?role=student&state=materials` @375 `presence` the FeatureManager list, replaced
...
- `/dev/ideacad?role=student&state=materials` @375 `presence` the viewport’s confirm pair, withdrawn while the panel is up
...
- `/dev/ideacad?role=student&state=materials` @1440 `prepare-wait` () => !!document.querySelector("[data-testid=\"ideacad-mater
- `/dev/ideacad?role=student&state=materials` @1440 `contrast` the panel heading on the pane ground
- `/dev/ideacad?role=student&state=materials` @1440 `contrast` a control label
- `/dev/ideacad?role=student&state=materials` @1440 `contrast` the density readout beside a picker
- `/dev/ideacad?role=student&state=materials` @1440 `contrast` the panel’s own prose
- `/dev/ideacad?role=student&state=materials` @1440 `contrast` the UNVERIFIED chip beside a density
- `/dev/ideacad?role=student&state=materials` @1440 `tap-target` a material or thickness picker
- `/dev/ideacad?role=student&state=materials` @1440 `tap-target` the confirm pair
- `/dev/ideacad?role=student&state=materials` @1440 `tap-target` the way back to the tree
...
- `/dev/ideacad?role=student&state=materials` @1440 `presence` the Materials panel
- `/dev/ideacad?role=student&state=materials` @1440 `presence` the FeatureManager list, replaced
- `/dev/ideacad?role=student&state=materials` @1440 `presence` body material, blade material, blade thickness, spin
- `/dev/ideacad?role=student&state=materials` @1440 `presence` the body fill slider
- `/dev/ideacad?role=student&state=materials` @1440 `presence` the body and blade density lines
- `/dev/ideacad?role=student&state=materials` @1440 `presence` the green check and the red X
- `/dev/ideacad?role=student&state=materials` @1440 `presence` Add my own material
- `/dev/ideacad?role=student&state=materials` @1440 `presence` the viewport’s confirm pair, withdrawn while the panel is up
...
- `/dev/ideacad?role=student&state=predicted` @375 `tap-target` a compare-sheet control
- `/dev/ideacad?role=student&state=predicted` @375 `distinguishable` a readout label against its own value
- `/dev/ideacad?role=student&state=predicted` @1440 `tap-target` a compare-sheet control
...
- `/dev/ideacad?role=student&state=property` @1440 `tap-target` a reorder control
- `/dev/ideacad?role=student&state=property` @1440 `tap-target` the way back to the tree
...
- `/dev/ideacad?role=student&state=three` @375 `tap-target` a concept card
- `/dev/ideacad?role=student&state=three` @375 `tap-target` a concept strip control
...
- `/dev/ideacad?role=student&state=three` @1440 `tap-target` a concept card
- `/dev/ideacad?role=student&state=three` @1440 `tap-target` a concept strip control
...
- `/dev/ideacad?role=teacher&state=property` @1440 `tap-target` the way back to the tree
...
- `/dev/ideacad` @375 `tap-target` a view toolbar control
- `/dev/ideacad` @375 `tap-target` a FeatureManager row
- `/dev/ideacad` @375 `tap-target` a concept strip control
- `/dev/ideacad` @375 `tap-target` Accept and Cancel
...
- `/dev/ideacad` @375 `order-result` the standard-view list, opened through its own control
...
- `/dev/ideacad` @1440 `tap-target` a view toolbar control
- `/dev/ideacad` @1440 `tap-target` a FeatureManager row
- `/dev/ideacad` @1440 `tap-target` a concept strip control
- `/dev/ideacad` @1440 `tap-target` Accept and Cancel
- `/dev/ideacad` @1440 `order-result` the console, the viewport and the view toolbar are where they claim to be
- `/dev/ideacad` @1440 `order-result` the standard-view list, opened through its own control
- `/dev/ideacad` @1440 `layout-sanity` the IdeaCAD console

<!-- counts:measured:data {"schema":3,"date":"2026-09-14T06:37:41.546Z","sha":"5399ccf32870e9df7f3de7e66aa88a288a4c6d66","dirty":true,"oldest":"2026-09-12T05:18:26.073Z","covered":["animated-logo-room.mjs","animated-logo.mjs","assignment-mirror-state-conflict.mjs","assignment-mirror.mjs","attach-reach.mjs","avatars.mjs","check-in-manage.mjs","classroom-deck.mjs","classroom-images.mjs","classroom-inspector-case-assignment-layout-0.mjs","classroom-inspector-case-assignment-layout-1.mjs","classroom-inspector-case-assignment-open-0.mjs","classroom-inspector-case-assignment-open-1.mjs","classroom-inspector-case-assignment-placement-top.mjs","classroom-inspector-case-sparse-open-1.mjs","classroom-interaction-case-fresh.mjs","classroom-interaction-case-typing.mjs","classroom-nav-manage-0.mjs","classroom-nav.mjs","classroom-split-s-1-item-i-crowded-manage-1-state-collapsed.mjs","classroom-split-s-1-item-i-crowded-manage-1.mjs","classroom-split-s-1-manage-1-state-compose-assignment-rubric.mjs","classroom-split-s-1-manage-1-state-selected.mjs","classroom-split-s-1-manage-1-state-units.mjs","classroom-split-s-1-manage-1.mjs","classroom-stream-manage-1.mjs","classroom-tools-live-stalled.mjs","classroom-tools-open-hall-pass-scope-manager.mjs","classroom-tools-open-hall-pass.mjs","classroom-tools-open-song-queue.mjs","classroom-tools.mjs","classroom-upload.mjs","classroom-view-as-notebook.mjs","classroom-view-class-bulk-state-no-units.mjs","classroom-view-class-bulk-state-selected.mjs","classroom-view-class-bulk-student.mjs","classroom-view-class-bulk.mjs","classroom-view-class-teacher.mjs","coin-desk-area-economy.mjs","coin-desk-state-picker.mjs","coin-desk.mjs","coin-preview.mjs","coins-signedin-1.mjs","coins.mjs","composer-attach.mjs","composer-draft.mjs","duplicate-drafts-state-empty.mjs","duplicate-drafts.mjs","feedback-state-dictating.mjs","feedback.mjs","foundry-admin-refusal.mjs","foundry-admin.mjs","foundry-covers.mjs","foundry-forge.mjs","foundry-gallery-state-detail-stats.mjs","foundry-gallery-state-full-screen.mjs","foundry-gallery.mjs","foundry-mosaic.mjs","foundry-submit.mjs","frc-state-review-console.mjs","frc-state-reviewer.mjs","frc-state-student.mjs","frc.mjs","gauntlet-rank-state.mjs","gauntlet-run.mjs","gauntlet-shell-countdown.mjs","gauntlet-shell.mjs","grading-bulk-case-batch.mjs","grading-bulk-leak-1.mjs","grading-bulk-state-single.mjs","grading-bulk.mjs","grading-change-state-pre-0171.mjs","grading-change-state-selected.mjs","grading-change.mjs","grading-incomplete-state-exports.mjs","grading-incomplete.mjs","grading-rubric-state-edited.mjs","grading-rubric.mjs","grading.mjs","greenline-portal-view-garage-seed-approved.mjs","greenline-portal-view-garage-seed-pending-panel-livery.mjs","greenline-portal-view-garage-seed-pending.mjs","greenline-portal-view-garage-seed-rejected.mjs","greenline-portal-view-moderation-seed-pending.mjs","greenline-portal-view-moderation.mjs","greenline-portal.mjs","hall-pass.mjs","home-feed-teacher.mjs","home-feed.mjs","home-order-role-student-classes-0-rows-3.mjs","home-order-role-student-classes-1-rows-3.mjs","home-order-role-student-classes-1-rows-4-due-1-0-1-5.mjs","home-order-role-teacher-classes-1-rows-3.mjs","html-assignment-doc-worksheet.mjs","html-assignment-grading-live-stalled.mjs","html-assignment-grading-state-broken.mjs","html-assignment-grading-state-closed.mjs","html-assignment-grading-state-empty.mjs","html-assignment-grading.mjs","html-assignment.mjs","html-instructor-copy.mjs","html-instructor.mjs","html-progress.mjs","html-rubric-state-graded.mjs","html-rubric-state-single.mjs","html-rubric.mjs","ideacad-archive-state-readonly.mjs","ideacad-archive.mjs","ideacad-attach-role-student-state-on.mjs","ideacad-attach-role-teacher-state-badconfig.mjs","ideacad-attach-role-teacher-state-off.mjs","ideacad-attach-role-teacher-state-on.mjs","ideacad-attach-role-teacher-state-other.mjs","ideacad-item-role-editor-state-lost.mjs","ideacad-item-role-editor.mjs","ideacad-item-role-owner-state-conflict.mjs","ideacad-item-role-owner.mjs","ideacad-item-role-viewer-state-callbacks.mjs","ideacad-item-role-viewer.mjs","ideacad-item-state-empty.mjs","ideacad-item-state-unavailable.mjs","ideacad-role-student-state-compare.mjs","ideacad-role-student-state-history.mjs","ideacad-role-student-state-materials.mjs","ideacad-role-student-state-predicted.mjs","ideacad-role-student-state-property.mjs","ideacad-role-student-state-three.mjs","ideacad-role-teacher-state-property.mjs","ideacad-shared-role-editor-state-lost.mjs","ideacad-shared-role-editor.mjs","ideacad-shared-role-viewer-state-callbacks.mjs","ideacad-shared-role-viewer.mjs","ideacad-shared-state-empty.mjs","ideacad-shared-state-unavailable.mjs","ideacad-team-role-editor-state-expiring.mjs","ideacad-team-role-editor-state-lost.mjs","ideacad-team-role-owner.mjs","ideacad-team-role-viewer.mjs","ideacad.mjs","instructor-requests.mjs","instructor-tools-class-big-tool-email.mjs","instructor-tools-tool-email.mjs","instructor-tools-tool-picker.mjs","instructor-tools.mjs","item-images-empty.mjs","item-images-text.mjs","item-images.mjs","maps-edit-state-compartment.mjs","maps-edit-state-duplicates.mjs","maps-edit-state-node-pending.mjs","maps-edit-state-place.mjs","maps-edit-state-unit.mjs","maps-edit.mjs","maps-editor-state-compartment.mjs","maps-editor-state-new-room.mjs","maps-editor-state-room.mjs","maps-editor-state-root.mjs","maps-editor.mjs","maps-grants-state-granted.mjs","maps-grants-state-published.mjs","maps-grants.mjs","maps-media-photos.mjs","maps-media.mjs","maps-shelf-state-no-photos.mjs","maps-shelf.mjs","maps-viewer-state-compartment.mjs","maps-viewer-state-item.mjs","maps-viewer-state-room.mjs","maps-viewer-state-stage-end.mjs","maps-viewer-state-stage-unit.mjs","maps-viewer-state-thin-stack.mjs","maps-viewer-state-unit.mjs","maps-viewer.mjs","marks.mjs","navigation-force-1.mjs","navigation-room-nb.mjs","navigation-room.mjs","navigation.mjs","notebook-grid-insert.mjs","notebook-plate-matrix.mjs","notebook-review-realtime-stalled.mjs","notebook-review-student.mjs","notebook-review-viewer-instructor-nosections-1.mjs","notebook-review-viewer-instructor.mjs","notebook-review-viewer-reviewer.mjs","notebook-review.mjs","notebook-sheet.mjs","notebook-site-matrix.mjs","notebook.mjs","pathways.mjs","portal-admin-owner-1.mjs","portal-admin-used-roster-admins.mjs","portal-admin.mjs","presence-presence-off.mjs","presence.mjs","profile-menu-state-open.mjs","profile-menu.mjs","short-links.mjs","song-queue.mjs","spec-importer-case-assignment.mjs","spec-importer-case-staging.mjs","spec-table-empty-1.mjs","spec-table-open.mjs","spec-table-rows-12.mjs","spec-table.mjs","themes-signedout-1.mjs","themes-state-matrix-room-classroom.mjs","themes-state-matrix-room-foundry.mjs","themes-state-matrix.mjs","themes.mjs","tournament-thumbs.mjs","tournaments-view-host-field-8-state-live-scores-1.mjs","tournaments-view-host-field-8-state-live.mjs","tournaments-view-list-signedin-1-admin-1.mjs","tournaments-view-list-signedin-1.mjs","tournaments-view-list.mjs","tournaments-view-page-field-16-state-live.mjs","tournaments-view-page-field-4-state-live.mjs","tournaments-view-page-field-8-state-live.mjs","tournaments-view-register-team-2.mjs","tournaments-view-team-state-live.mjs","tournaments-view-team-state-open-team-3.mjs","tournaments-view-team-state-open-viewer-host.mjs","tournaments-view-team-state-open.mjs","tournaments-view-tv-status-live-field-4-state-live.mjs","tournaments-view-tv-status-live-field-8-state-live.mjs","tournaments-view-tv-status-live-field-8.mjs","tournaments-view-tv-status-registration-open-field-22.mjs","tournaments.mjs","upload-limits.mjs"],"runsMeasured":460,"measurements":8312,"outside":147,"outsideRows":[{"path":"/dev/ideacad-item?role=editor&state=lost","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad-item?role=editor&state=lost","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"},{"path":"/dev/ideacad-item?role=editor","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad-item?role=editor","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"},{"path":"/dev/ideacad-item?role=owner&state=conflict","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad-item?role=owner&state=conflict","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"},{"path":"/dev/ideacad-item?role=owner","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad-item?role=owner","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"},{"path":"/dev/ideacad-item?role=viewer&state=callbacks","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad-item?role=viewer&state=callbacks","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"},{"path":"/dev/ideacad-item?role=viewer","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad-item?role=viewer","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"},{"path":"/dev/ideacad-item?state=empty","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad-item?state=empty","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"},{"path":"/dev/ideacad?role=student&state=compare","width":375,"check":"tap-target","label":"the concept picker"},{"path":"/dev/ideacad?role=student&state=compare","width":375,"check":"tap-target","label":"the say-why field"},{"path":"/dev/ideacad?role=student&state=compare","width":375,"check":"tap-target","label":"a compare-sheet control"},{"path":"/dev/ideacad?role=student&state=compare","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad?role=student&state=compare","width":1440,"check":"tap-target","label":"the concept picker"},{"path":"/dev/ideacad?role=student&state=compare","width":1440,"check":"tap-target","label":"the say-why field"},{"path":"/dev/ideacad?role=student&state=compare","width":1440,"check":"tap-target","label":"a compare-sheet control"},{"path":"/dev/ideacad?role=student&state=compare","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"},{"path":"/dev/ideacad?role=student&state=history","width":375,"check":"tap-target","label":"a step, which is what a scrub is pressed on"},{"path":"/dev/ideacad?role=student&state=history","width":375,"check":"tap-target","label":"Undo and Redo inside the panel"},{"path":"/dev/ideacad?role=student&state=history","width":375,"check":"tap-target","label":"the way back to the feature tree"},{"path":"/dev/ideacad?role=student&state=history","width":375,"check":"tap-target","label":"the History toggle in the header"},{"path":"/dev/ideacad?role=student&state=history","width":375,"check":"presence","label":"the history panel"},{"path":"/dev/ideacad?role=student&state=history","width":375,"check":"presence","label":"the step count, which stands in for the scrollbar nobody paints"},{"path":"/dev/ideacad?role=student&state=history","width":375,"check":"presence","label":"Undo, inside the panel"},{"path":"/dev/ideacad?role=student&state=history","width":375,"check":"presence","label":"Redo, inside the panel"},{"path":"/dev/ideacad?role=student&state=history","width":375,"check":"presence","label":"the undone step, still listed"},{"path":"/dev/ideacad?role=student&state=history","width":375,"check":"presence","label":"the creation of the part"},{"path":"/dev/ideacad?role=student&state=history","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad?role=student&state=history","width":1440,"check":"tap-target","label":"Undo and Redo inside the panel"},{"path":"/dev/ideacad?role=student&state=history","width":1440,"check":"tap-target","label":"the way back to the feature tree"},{"path":"/dev/ideacad?role=student&state=history","width":1440,"check":"tap-target","label":"the History toggle in the header"},{"path":"/dev/ideacad?role=student&state=history","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"prepare-wait","label":"() => !!document.querySelector(\"[data-testid=\\\"ideacad-mater"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"contrast","label":"the panel heading on the pane ground"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"contrast","label":"a control label"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"contrast","label":"the density readout beside a picker"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"contrast","label":"the panel’s own prose"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"contrast","label":"the UNVERIFIED chip beside a density"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"tap-target","label":"a material or thickness picker"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"tap-target","label":"the confirm pair"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"tap-target","label":"the way back to the tree"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"tap-target","label":"Add my own material"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"tap-target","label":"the body fill slider"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"presence","label":"the Materials panel"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"presence","label":"the FeatureManager list, replaced"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"presence","label":"body material, blade material, blade thickness, spin"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"presence","label":"the body fill slider"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"presence","label":"the body and blade density lines"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"presence","label":"the green check and the red X"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"presence","label":"Add my own material"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"presence","label":"the viewport’s confirm pair, withdrawn while the panel is up"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"text-contains","label":"the panel states the thickness rule and the UNVERIFIED density rather than leaving either to be discovered"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"order-result","label":"the panel replaced the tree in the tree’s own pane, and the controls drive the readouts"},{"path":"/dev/ideacad?role=student&state=materials","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"prepare-wait","label":"() => !!document.querySelector(\"[data-testid=\\\"ideacad-mater"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"contrast","label":"the panel heading on the pane ground"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"contrast","label":"a control label"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"contrast","label":"the density readout beside a picker"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"contrast","label":"the panel’s own prose"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"contrast","label":"the UNVERIFIED chip beside a density"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"tap-target","label":"a material or thickness picker"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"tap-target","label":"the confirm pair"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"tap-target","label":"the way back to the tree"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"tap-target","label":"Add my own material"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"tap-target","label":"the body fill slider"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"presence","label":"the Materials panel"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"presence","label":"the FeatureManager list, replaced"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"presence","label":"body material, blade material, blade thickness, spin"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"presence","label":"the body fill slider"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"presence","label":"the body and blade density lines"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"presence","label":"the green check and the red X"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"presence","label":"Add my own material"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"presence","label":"the viewport’s confirm pair, withdrawn while the panel is up"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"text-contains","label":"the panel states the thickness rule and the UNVERIFIED density rather than leaving either to be discovered"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"order-result","label":"the panel replaced the tree in the tree’s own pane, and the controls drive the readouts"},{"path":"/dev/ideacad?role=student&state=materials","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"},{"path":"/dev/ideacad?role=student&state=predicted","width":375,"check":"tap-target","label":"a compare-sheet control"},{"path":"/dev/ideacad?role=student&state=predicted","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad?role=student&state=predicted","width":1440,"check":"tap-target","label":"a compare-sheet control"},{"path":"/dev/ideacad?role=student&state=predicted","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"contrast","label":"the panel’s own prose"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"tap-target","label":"the confirm pair"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"tap-target","label":"a reorder control"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"tap-target","label":"the way back to the tree"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"tap-target","label":"a station cell"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"tap-target","label":"add or remove a station"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"presence","label":"the PropertyManager"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"presence","label":"station rows"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"presence","label":"the r and z cells"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"presence","label":"add and remove, one pair per station"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"presence","label":"the 2D profile preview"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"presence","label":"one dot per station on the profile"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"presence","label":"the green check and the red X"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"presence","label":"Move up and Move down"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"presence","label":"the sentence saying why no feature renames or deletes"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"text-contains","label":"the panel names its bounds rather than leaving them to be discovered"},{"path":"/dev/ideacad?role=student&state=property","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad?role=student&state=property","width":1440,"check":"contrast","label":"the panel’s own prose"},{"path":"/dev/ideacad?role=student&state=property","width":1440,"check":"tap-target","label":"the confirm pair"},{"path":"/dev/ideacad?role=student&state=property","width":1440,"check":"tap-target","label":"a reorder control"},{"path":"/dev/ideacad?role=student&state=property","width":1440,"check":"tap-target","label":"the way back to the tree"},{"path":"/dev/ideacad?role=student&state=property","width":1440,"check":"tap-target","label":"a station cell"},{"path":"/dev/ideacad?role=student&state=property","width":1440,"check":"tap-target","label":"add or remove a station"},{"path":"/dev/ideacad?role=student&state=property","width":1440,"check":"presence","label":"the sentence saying why no feature renames or deletes"},{"path":"/dev/ideacad?role=student&state=property","width":1440,"check":"text-contains","label":"the panel names its bounds rather than leaving them to be discovered"},{"path":"/dev/ideacad?role=student&state=property","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"},{"path":"/dev/ideacad?role=student&state=three","width":375,"check":"tap-target","label":"a concept card"},{"path":"/dev/ideacad?role=student&state=three","width":375,"check":"tap-target","label":"a concept strip control"},{"path":"/dev/ideacad?role=student&state=three","width":375,"check":"presence","label":"the failing rule chip"},{"path":"/dev/ideacad?role=student&state=three","width":375,"check":"presence","label":"the passing rule chips"},{"path":"/dev/ideacad?role=student&state=three","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad?role=student&state=three","width":1440,"check":"tap-target","label":"a concept card"},{"path":"/dev/ideacad?role=student&state=three","width":1440,"check":"tap-target","label":"a concept strip control"},{"path":"/dev/ideacad?role=student&state=three","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"},{"path":"/dev/ideacad?role=teacher&state=property","width":375,"check":"tap-target","label":"the way back to the tree"},{"path":"/dev/ideacad?role=teacher&state=property","width":375,"check":"presence","label":"the PropertyManager"},{"path":"/dev/ideacad?role=teacher&state=property","width":375,"check":"presence","label":"station rows, the same four"},{"path":"/dev/ideacad?role=teacher&state=property","width":375,"check":"presence","label":"the r and z cells, shown and disabled"},{"path":"/dev/ideacad?role=teacher&state=property","width":375,"check":"presence","label":"one dot per station on the profile"},{"path":"/dev/ideacad?role=teacher&state=property","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad?role=teacher&state=property","width":1440,"check":"tap-target","label":"the way back to the tree"},{"path":"/dev/ideacad?role=teacher&state=property","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"},{"path":"/dev/ideacad","width":375,"check":"contrast","label":"the sentence saying what the tree does not offer"},{"path":"/dev/ideacad","width":375,"check":"tap-target","label":"a view toolbar control"},{"path":"/dev/ideacad","width":375,"check":"tap-target","label":"a FeatureManager row"},{"path":"/dev/ideacad","width":375,"check":"tap-target","label":"a concept strip control"},{"path":"/dev/ideacad","width":375,"check":"tap-target","label":"Accept and Cancel"},{"path":"/dev/ideacad","width":375,"check":"presence","label":"FeatureManager rows"},{"path":"/dev/ideacad","width":375,"check":"presence","label":"FeatureManager rows and the body’s expander"},{"path":"/dev/ideacad","width":375,"check":"presence","label":"the body’s expander"},{"path":"/dev/ideacad","width":375,"check":"presence","label":"rule readouts, the centre of mass and the physics"},{"path":"/dev/ideacad","width":375,"check":"order-result","label":"the console, the viewport and the view toolbar are where they claim to be"},{"path":"/dev/ideacad","width":375,"check":"order-result","label":"the standard-view list, opened through its own control"},{"path":"/dev/ideacad","width":375,"check":"distinguishable","label":"a readout label against its own value"},{"path":"/dev/ideacad","width":1440,"check":"contrast","label":"the sentence saying what the tree does not offer"},{"path":"/dev/ideacad","width":1440,"check":"tap-target","label":"a view toolbar control"},{"path":"/dev/ideacad","width":1440,"check":"tap-target","label":"a FeatureManager row"},{"path":"/dev/ideacad","width":1440,"check":"tap-target","label":"a concept strip control"},{"path":"/dev/ideacad","width":1440,"check":"tap-target","label":"Accept and Cancel"},{"path":"/dev/ideacad","width":1440,"check":"order-result","label":"the console, the viewport and the view toolbar are where they claim to be"},{"path":"/dev/ideacad","width":1440,"check":"order-result","label":"the standard-view list, opened through its own control"},{"path":"/dev/ideacad","width":1440,"check":"layout-sanity","label":"the IdeaCAD console"}],"totalMs":1289779,"selftest":{"controls":98,"negative":50,"positive":48,"failures":0}} -->
<!-- counts:measured:end -->
...
false green.** At commit `799d203` the IdeaCAD 3D viewport was a canvas
measuring 912.0 x 0.0 whose drawing buffer was 1368 x 1, on every editor route
at both widths -- a completely black viewport -- and the route's own presence,
contrast, tap-target and text-contains rows were all green over it. "Present,
visible, large enough, readable" is a claim about a BOX, and a box can be
perfect over nothing at all. `main` has since repaired that pane, which is why
`canvas-content` reads 16.87% to 42.83% off the dominant colour over 6,833 to
9,706 distinct colours across its thirty readings today; the same check on the
same route at `799d203` read 0.00% over 6. Three details of their own:
...
**MEASURED ON `/dev/ideacad?role=teacher&state=property` AT 1440**, with each
preset's own row in bold and every other row at its baseline -- which is the
property that makes a preset worth anything:
...
**A LIVE CONTROL CANNOT SHOW GREEN-TO-RED ON A CHECK THAT IS ALREADY RED, AND
`zero-box` IS IN THAT POSITION** -- the surface genuinely has three zero-box
controls -- so what it demonstrates is the count moving 3 to 6, with the newly
zeroed controls named by their own text. The other two are clean transitions.
The same was true of `blank-canvas` at `799d203`, where `canvas-content`
short-circuited on the canvas's zero CSS box before it read a pixel; `main`
repaired that pane, and the transition is available on the route now.
