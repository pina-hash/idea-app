...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 276 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 101 |
| Directories under `src/routes/dev` with a page (the candidate set) | 127 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 552 |

<!-- counts:static:data {"schema":1,"specs":276,"routes":101,"devPages":127,"widths":[375,1440],"runs":552} -->
<!-- counts:static:end -->
...
| Count | Value |
| --- | --- |
| Route specs measured (one file each under `measured/`) | 276 |
| Route/width runs those measurements cover | 552 |
| Measurements | 9978 |
| Measurements outside threshold | 182 |
| Wall clock, summed from each spec's attributed share | 1608.2s |
