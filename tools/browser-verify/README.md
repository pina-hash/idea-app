...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 279 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 102 |
| Directories under `src/routes/dev` with a page (the candidate set) | 128 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 558 |

<!-- counts:static:data {"schema":1,"specs":279,"routes":102,"devPages":128,"widths":[375,1440],"runs":558} -->
<!-- counts:static:end -->
...
| Count | Value |
| --- | --- |
| Route specs measured (one file each under `measured/`) | 279 |
| Route/width runs those measurements cover | 558 |
| Measurements | 10076 |
| Measurements outside threshold | 182 |
| Wall clock, summed from each spec's attributed share | 1655.7s |
