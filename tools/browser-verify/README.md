...
<!-- counts:begin -->
**Generated by `npm run verify:readme`; do not edit by hand.** Measured 2026-09-03T15:19:30.798Z on commit `f60c70c` by a full run of `tools/browser-verify/run.mjs` in this container. `tests/derived-numbers.test.ts` reddens on a hand edit or on a spec added without regenerating.

| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 80 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 43 |
| Directories under `src/routes/dev` with a page (the candidate set) | 75 |
| Widths | 2 (375, 1440) |
| Route/width runs (specs x widths) | 160 |
| Route/width runs the report carried | 160 |
| Measurements | 2196 |
| Measurements outside threshold | 8 |
| Full-run wall clock | 371.0s |
