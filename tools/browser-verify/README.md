...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 117 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 58 |
| Directories under `src/routes/dev` with a page (the candidate set) | 87 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 234 |

<!-- counts:static:data {"schema":1,"specs":117,"routes":58,"devPages":87,"widths":[375,1440],"runs":234} -->
<!-- counts:static:end -->

### Measured -- from a full harness run

<!-- counts:measured:begin -->
**Generated by `npm run verify:readme`; do not edit by hand.** Measured 2026-09-06T08:03:44.229Z on commit `72f1fa5` (working tree dirty at measurement) by a full run of `tools/browser-verify/run.mjs` in this container. It needs a browser and about six minutes, so it is regenerated deliberately and not on every branch; a stale-but-honest measured half is a supported state.
...
| Count | Value |
| --- | --- |
| Route specs the run covered | 117 |
| Route/width runs the report carried | 234 |
| Measurements | 3474 |
| Measurements outside threshold | 2 |
| Full-run wall clock | 566.6s |
