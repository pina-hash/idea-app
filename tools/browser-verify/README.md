...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 118 |
...
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 236 |

<!-- counts:static:data {"schema":1,"specs":118,"routes":58,"devPages":87,"widths":[375,1440],"runs":236} -->
<!-- counts:static:end -->

### Measured -- from a full harness run

<!-- counts:measured:begin -->
**Generated by `npm run verify:readme`; do not edit by hand.** Measured 2026-09-06T08:21:47.235Z on commit `f2a5d8f` by a full run of `tools/browser-verify/run.mjs` in this container. It needs a browser and about six minutes, so it is regenerated deliberately and not on every branch; a stale-but-honest measured half is a supported state.
...
| Count | Value |
| --- | --- |
| Route specs the run covered | 118 |
| Route/width runs the report carried | 236 |
| Measurements | 3464 |
| Measurements outside threshold | 2 |
| Full-run wall clock | 609.5s |
