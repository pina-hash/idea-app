...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 202 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 81 |
| Directories under `src/routes/dev` with a page (the candidate set) | 109 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 404 |

<!-- counts:static:data {"schema":1,"specs":202,"routes":81,"devPages":109,"widths":[375,1440],"runs":404} -->
<!-- counts:static:end -->

### Measured -- summed from `measured/`, one file per route spec

<!-- counts:measured:begin -->
**Generated by `npm run verify:counts` from `measured/`; do not edit by hand.** Every route spec carries its OWN measurement file there, written by `npm run verify:readme` -- which needs a browser and about seventeen minutes for the whole tree, and seconds for one spec with `-- --route <spec>`. This table is the sum over those files, so it is a tree read like the static region above and costs nothing to regenerate on a merged tree. The newest measurement here was taken on commit `9ee0110` (a working tree was dirty at measurement); measurements were taken between 2026-09-12T05:18:26.073Z and 2026-09-12T20:54:48.349Z. A stale-but-honest measured half is a supported state.
...
| Count | Value |
| --- | --- |
| Route specs measured (one file each under `measured/`) | 202 |
| Route/width runs those measurements cover | 404 |
| Measurements | 7184 |
| Measurements outside threshold | 0 |
| Wall clock, summed from each spec's attributed share | 1096.5s |
