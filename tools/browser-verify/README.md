...
| Count | Value |
| --- | --- |
| Route specs (`routes/*.mjs`, `_`-prefixed excluded) | 223 |
| Distinct routes those specs drive (alias-resolved, query string stripped) | 86 |
| Directories under `src/routes/dev` with a page (the candidate set) | 114 |
| Widths | 2 (375, 1440) |
| Route/width runs a full pass makes (specs x widths) | 446 |

<!-- counts:static:data {"schema":1,"specs":223,"routes":86,"devPages":114,"widths":[375,1440],"runs":446} -->
<!-- counts:static:end -->
...
| Count | Value |
| --- | --- |
| Route specs measured (one file each under `measured/`) | 223 |
| Route/width runs those measurements cover | 446 |
| Measurements | 7968 |
| Measurements outside threshold | 0 |
| Wall clock, summed from each spec's attributed share | 1246.1s |
