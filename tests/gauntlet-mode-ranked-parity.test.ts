...
async function seedPassingRun(mode: GauntletModeId) {
	...
	await db.sql(
		`insert into public.submissions (user_id, challenge_id, mode, value, is_correct, score_metric, source)
		 values ($1::uuid, $2::uuid, $3::public.gauntlet_mode, '{}'::jsonb, true, 1, $4)`,
		[player.id, challengeIds.get(mode)!, mode, source]
	);
}
