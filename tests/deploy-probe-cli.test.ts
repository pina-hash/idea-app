...
describe('the CLI, end to end, against a real database', () => {
	...
	it('reads the seeded table, and reports the record on its own first line', async () => {
		...
		expect(r.out).toContain('migration  record      state        read from  object');
		expect(r.err).toBe('');
	}, SLOW);
	...
	it('A ROW IS NEVER A LICENCE WHERE A PROBE ACTUALLY RAN: every claim is refused', async () => {
		...
		const conflicts = j.findings.filter((f) => f.agreement === 'conflict');
	...
	it('names every CONFLICT in the text report rather than counting it', () => {
		const r = probe(urlFor(db!), ['--since', '205', '--ref', 'HEAD']);
		expect(r.out).toMatch(
			/CONFLICT\(S\): a row in supabase_migrations\.schema_migrations claims an apply whose object is NOT there:/
		);
		expect(r.out).toMatch(/^\d+ CONFLICT\(S\): supabase_migrations\.schema_migrations claims an apply/m);
	...
	it('ENDS STATUS 3: a migration with no derivable probe answers from its row, and exits 0', async () => {
		...
		try {
			...
			// stopped lane after lane, reproduced exactly.
			...
			// THE SERIALIZED `readFrom`, WHICH `deploy.yml` PRINTS. It is in the
			// payload rather than re-derived in `jq`, so this is the one place
			// the workflow's column is pinned end to end.
			expect(ja.findings[0].evidence).toBe('unknown');
			expect(ja.findings[0].readFrom).toBe('history row');
	...
	it('prints all three statements under --print-sql and runs nothing', () => {
		...
		expect(r.out).toContain(`version from ${HISTORY_TABLE}`);
