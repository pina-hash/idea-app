...
// the panel asks the workspace to do. The solver is proven in
// `tests/ideacad-solid-mates-*.test.ts`; what is here is the half that only
// exists once the component is wired:
//
//   * Add applies `{type:'add-feature', feature:{type:'mate', kind, a, b}}`
...
//   * the list shows every mate with a status word, its residual and its
//     message, and Delete applies `{type:'remove-feature'}`;
...
//   * the freedom sentence per mated body, and Fix in place as a metadata
//     command;
...
describe('the mate panel', () => {
	it('with fewer than two picks, Add refuses through api.error and applies nothing; with two picks on two bodies it applies the mate, first pick first', async () => {
		...
		expect(m.one('[data-testid="ideacad-mate-picks"]').textContent).toContain('The first pick stays; the second moves to it.');
		await add(m);
		expect(h.applied).toHaveLength(0);
		expect(h.errors).toEqual(['Select a face, edge or corner on one body, then shift-click one on another body. The first pick stays; the second moves to it.']);
		h.set({ selections: [pick('x1#0', 'x1.end'), pick('x2#0', 'x2.start')] }); m.flush();
		expect(m.one('[data-testid="ideacad-mate-picks"]').textContent).toBe('First (stays): Base, x1.end (face). Second (moves): Bracket, x2.start (face).');
		await add(m);
	...
	it('two picks on one body are refused naming the body; flip and a reference pick go through', async () => {
		const h = harness({ selections: [pick('x1#0', 'x1.end'), pick('x1#0', 'x1.start')] }); const m = mountPanel(h);
		await add(m);
		expect(h.applied).toHaveLength(0); expect(h.errors).toEqual(['Mate two different bodies. Both picks are on Base.']);
		...
		const flip = m.one<HTMLInputElement>('form.create input[type="checkbox"]'); expect(flip.closest('label')!.textContent).toContain('Flip'); flip.click(); m.flush();
		await add(m);
		expect(h.applied).toHaveLength(1);
		...
		expect(m.one('[data-testid="ideacad-mate-picks"]').textContent).toContain('First (stays): Plane 1 (reference)');
	});
	it('distance and angle take a value: a non-number is refused in a sentence, a number goes through as typed, and the input carries no min, max or step', async () => {
		const h = harness({ selections: [pick('x1#0', 'x1.end'), pick('x2#0', 'x2.start')] }); const m = mountPanel(h);
	...
	it('lists every mate with a status word, its residual and its sentence; Delete removes the feature and Set patches the value', async () => {
		...
		expect(rows[0].querySelector('.status')!.textContent).toBe('Holds'); expect(rows[0].classList.contains('ok')).toBe(true);
		expect(rows[0].querySelector('strong')!.textContent).toBe('Mate 1'); expect(rows[0].querySelector('.kind')!.textContent).toBe('Coincident');
		expect(rows[0].querySelector('.sides')!.textContent).toBe('Base x1.end to Bracket x2.start');
		expect(rows[0].querySelector('.residual')!.textContent).toBe('Off by 0 in');
		expect(rows[0].querySelector('.message')).toBeNull();
		expect(rows[1].querySelector('.status')!.textContent).toBe('Not solved'); expect(rows[1].classList.contains('error')).toBe(true);
	...
	it('says what freedom each mated body has left, and Fix in place is a metadata command', async () => {
		...
		expect(m.all('.bodies li')).toHaveLength(0); expect(m.one('.note').textContent).toContain('No mates yet');
	});
});
